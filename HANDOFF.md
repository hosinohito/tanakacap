...
- 顔ヨーは通常PnP経路で円周上の両目の投影弦長からacosで復元する方式を実装。正面10観測で基準取得、PnPの眼中心面距離で接近を補正。鼻は左右判定のみ。頭のピッチ・ヨー・ロール共通で可変速度追従を有効化（UI既定・ローカル保存値・通常PSランチャー）。[仕様と限界](docs/HEAD_YAW_AUDIT.md)。最新901観測で数値確認、見た目は通常の録画UIで確認待ち。既存Playerで動作、再ビルド不要。
