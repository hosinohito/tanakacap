...
- 現在の課題は右手ロール時に左手もロールする／左指が動かない症状。指定の改変済みHAOLANで、左右の人工入力・開発/配布Player・既存録画を調査済み。状態共有は未検出。新録画901観測で左掌の生法線反転と、Unityの左前腕が+160度へ張り付く問題を確認。次は360度別表現の選択と可動制限の組み合わせを見直す。左右連動の直接原因はまだ未確定。[調査・次の切り分け](docs/HAND_SIDE_AUDIT.md)。動きの方式は未変更。明示起動時だけ有効な実骨ログを追加しPlayerを再ビルド済み。公開指示はない。
...
- **左右の手の連動・左指**：撮影用はデスクトップの`tanakacap-face-capture.bat`（30秒・案内なし・実写非表示）。最新takeは `results/comparison-takes/20260923T064052-167149Z`、解析・アバター動画は `results/hand-sides-new-recording-20260923/`。検証UIの入力/アバターも変更済み。右手だけの動作は6秒と15秒とユーザー確認済み。通常の実時間送受信でも左上限への張り付きを再現、掌/指の送受信値12,138項目に不一致なし。診断結果は `results/hand-realtime/20260923T065613-282164Z/`。次はユーザーがデスクトップ `tanakacap-tools/tanakacap-hands-camera.bat` で実カメラを試す。カメラはまだ起動していない。左前腕の境界への張り付きは通常再生でも確認済み、指は常時停止ではなく信頼度不足等による欠測を含む。人工入力の成功を症状の解決と扱わない。
