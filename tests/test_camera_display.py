...
import cv2
import pytest
from capture_lab import camera_display, __main__ as app, comparison_capture, head_only
...
def test_exact_startup_token_required(monkeypatch):
    for token in ('--preview', '--diagnose', camera_display.DISPLAY_FLAG[:35],
        ...
        monkeypatch.setattr(sys, 'argv', ['capture_lab', token])
        assert not camera_display.allowed()
    monkeypatch.setattr(sys, 'argv', ['capture_lab', camera_display.DISPLAY_FLAG])
    assert camera_display.allowed()


def test_all_display_sinks_closed_without_opt_in(monkeypatch):
    monkeypatch.setattr(sys, 'argv', ['capture_lab', '--preview', '--diagnose'])
...
def test_opt_in_can_display_and_does_not_persist(monkeypatch):
    ...
    monkeypatch.setattr(sys, 'argv', ['capture_lab'])
    camera_display.show('test', frame)
    assert len(seen)==1
...
def test_old_or_abbreviated_flags_rejected_before_execution(monkeypatch, flag):
    monkeypatch.setattr(sys, 'argv', ['capture_lab', 'benchmark', flag])
...
def test_raw_display_api_is_centralized():
    ...
    for folder in ('capture_lab','tools'):
    ...
    launcher=(root/'run-avatar-lab.ps1').read_text(encoding='utf-8-sig')
