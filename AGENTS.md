...
最新（2026-09-13）：UI推論同期はFPS欄なし、自由入力のみ表示。同期時は保存fpsを推論上限に使わない。プルダウン幅固定。実packet15/35Hz同期出力検証済み。配布実物監査RELEASE_LICENSE_AUDIT.md：通知14件とSDKヘッダー等除外、自動照合を追加、NVIDIA契約不一致/モデル重み/FFmpeg対応ソース等5残件で公開保留。外部問い合わせ未送信、モデルやCUDAをこの監査だけで交換しない。
