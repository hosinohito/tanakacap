...
| OpenCV | packages/opencv-python | 本体Apache-2.0、FFmpegはLGPL-2.1系。対応ソース・ビルド手順はsources/opencv-ffmpeg-sources.zip |
| NVIDIAライブラリ | packages/nvidia-* | SDK実行DLLのみ。公式同版ZIPとの照合済み。notices/NVIDIA-*.txtの公式契約も参照 |
...
| Unity | notices/Unity-2022.3.22f1-ThirdPartyNotices.txt | Player-Windows-Mono専用PDFも同梱。Editorを配布しているという意味ではない |
...
通知の取得元とSHAはnotices/sources.json。配布条件の根拠は開発リポジトリのdocs/RUNTIME_LICENSE_DECISIONS.md。コードが公開されていること、モデルをダウンロードできること、学習データの再配布権、生成した映像の権利は区別します。

SDK本体、TensorRT、比較専用SAM/HaMeR/MANO、実写録画、HAOLAN原本、OBS、開発用FFmpeg実行ファイルはこのZIPへ含めません。OpenCV内部のFFmpeg DLLは別の同梱部品です。

KlakSpoutのWIDL生成COMヘッダーの由来はWine-d3d11on12.idl、本文はWine-LGPL-2.1.txtを参照してください。Wineランタイムは同梱していません。
