...
ユーザーが条件説明に対して「使わない」と明示。試験接続と追加3パッケージを削除済み。CUDA FP16は継続。以下の条件確認待ち・接続準備の記述は判断過程の記録で、現在の再導入予定ではない。

### 最初の棚卸しと条件確認

更新：その後ユーザーは「CUDAは外せないので、TensorRTも同条件なら実装」と指定。通常版10.16.1.11/cu13の実wheel契約を確認したところ、下記Web最新の契約と一致しなかった。§12.2の1年自動更新/更新終了、§2.1(ix)の競合技術制限、§12.1の再配布対象の狭い記述を確認。CUDAと同条件とは未確定で、モデル実行・エンジン構築はユーザー確認待ち。導入は開発venvでの版/契約確認まで。詳細[精度形式比較](PRECISION_COMPARISON.md)。FP16は既存CUDA/ORT内で実装・比較を進める。以下の「未導入」「FP16も追加しない」は最初の判断であり現状ではない。
...
FP16比較後の追加棚卸しは`results/distribution-audit-precision-20260913`。25packages/51通知/19ONNX。TensorRT候補3packagesは契約確認のため開発環境へ追加したもの、製品同梱の承認ではない。FP16派生ONNXのSHAと実TensorRT wheel契約を保存。CUDA FP16は既存ORTのMIT変換器を使い、新たなSDK許諾を必要とする経路を追加しない。モデル原本の許諾・派生通知は引き続き必要。

## 配布までに残る具体作業

TensorRT削除後の現環境は`results/distribution-audit-no-tensorrt-20260913`へ再収集（22packages/48通知/19ONNX）。過去のTensorRT契約確認用inventoryは履歴資料として保持し、現在の製品依存とは区別する。
