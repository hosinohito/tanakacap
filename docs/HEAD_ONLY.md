...
- 直接：`.\run-avatar-lab.ps1 -Camera 1 -HeadOnly -NoLog`。通常のfull設定を変更しない。
- P：一時停止/再開。R：対象をリセットして自動取得。S：複数人や誤った対象の時に頭を矩形で選択。Q/Esc：終了。
- `-HeadRoi X,Y,W,H` はautoでは最初の対象指定。頭の動きとともに更新する。
- `-HeadRoiMode fixed` は従来の手動固定方式。頭領域モデルも起動せず、空の範囲の自動ロストはできない。引数なしなら最初に選択。CLIは `--head-roi-mode fixed --roi X Y W H`。autoではプレビューなし・範囲指定なしでも動く。
