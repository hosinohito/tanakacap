...
## 全保存録画での関節別検証（2026-09-23）

保存されている実写元動画4本・計7,842フレーム（タイムライン計約275秒）をすべて再解析。アバター比較動画・合成入力・同一録画の派生物は別撮影に数えない。元動画とタイムラインはload_takeでハッシュ確認、全フレームを復号。4本とも同じ現在のRTMW3D混合FP16/crop、YOLOX-MのCUDA graph、人物切り出し3フレーム間隔、現在のBodyRetargetの顔尺度推定で処理し、指は反転前66d08f2の計算を使用。指の集計は前節と同じ生の符号付き角度で、±7度（親指±5度）の帯を除く。負側割合は誤推定率ではない。顔表情やアバター描画は不要なため実行していない。

| 撮影日時（日本時間） | フレーム | 長さ | 左指の有効観測数（親・人・中・薬・小） |
| --- | --- | --- | --- |
| 9/12 08:53 | 5187 | 185.0秒 | 3145 / 2871 / 2359 / 2285 / 2806 |
| 9/13 08:05 | 853 | 30.0秒 | 7 / 0 / 0 / 0 / 0 |
| 9/21 13:49 | 901 | 30.0秒 | 628 / 562 / 520 / 485 / 519 |
| 9/23 15:40 | 901 | 30.0秒 | 693 / 698 / 659 / 658 / 817 |

9/13の録画は親指7観測のみ、他4指は有効観測0。全フレームを処理したが、傾向の結論には使用しない。残る3本の負側割合（%、付け根 / 中間 / 指先側、親指の中間は対象なし）：

| 左指 | 9/12 | 9/21 | 9/23 |
| --- | --- | --- | --- |
| 親指 | 8 / — / 44 | 6 / — / 32 | 5 / — / 42 |
| 人差し指 | 16 / 20 / 40 | 40 / 29 / 34 | 36 / 20 / 24 |
| 中指 | 22 / 32 / 32 | 40 / 34 / 34 | 54 / 25 / 21 |
| 薬指 | 22 / 34 / 53 | 41 / 35 / 35 | 40 / 30 / 39 |
| 小指 | 27 / 22 / 49 | 34 / 30 / 39 | 47 / 28 / 41 |

結論：動画・指・関節で符号傾向が変わる。9/12の人差し指〜小指MCPは正側50〜63%で、9/23の15〜26%より多い。親指MCPは有効観測の十分な3本とも正側84〜93%。中指DIPも3本とも負側より正側が多い。一律反転だけでなく、特定関節の恒久的な符号反転もこの分布だけでは正当化できない。手の姿勢・実際の曲げ動作を照合していないため、負側の多さをモデル故障や解釈の誤りと直接同一視しない。現行のユーザー指定反転試行は変更なし。

前節の9/23の数値との差は、今回は人物ROI/尺度も共通設定で再生成したため。前節は保存済みROI/尺度を使った別条件であり、精度の改善・悪化を意味しない。4本の比較には本節の統一条件を用いる。

再現：`.venv/Scripts/python.exe -X utf8 tools/diagnostics/audit_all_take_finger_signs.py --output <新規結果フォルダー>`。既定入力はresults/comparison-takes全件。結果は`results/finger-sign-all-takes-20260923-v2/report.json`、各撮影ID下に再推論観測frames.jsonl、audit/report.json（両手各関節の正負割合・中央値・5/95パーセンタイル）、audit/angles.jsonl。全観測の再生で計測フックあり/なしの旧計算packet一致を検査済み。観測0は0%と扱わずno_valid_observationsを保存。実写表示ガード8テスト成功。初回は診断用packetのfaceTracked初期化漏れで開始直後に停止し、診断ツール内を修正して新しいv2出力で全件再実行。製品コード変更・ビルド・公開なし。
