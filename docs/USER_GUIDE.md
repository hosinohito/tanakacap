...
体の大きさや目の動く範囲は、自動で調整されます。毎回、次のように始めてください。

1. カメラを正面に置き、普段使う位置・距離に座って開始します。
2. 両腕を横に大きく広げます。できるだけ両手までカメラに収まるようにしてください。
3. 目を大きく開き、視線を左右・上下に動かします。

「部位ごとの更新速度」は、1秒あたりに新しい動きが届く回数（fps）です。大きい数値ほどこまめに更新されています。「未検出」「保持中」のときは、新しい動きを取得できていません。

手が見えなくなったときは、0.5秒待ってから1秒かけて机に手を置く姿勢へ移ります。再び見えると、0.5秒かけて追従へ戻ります。実際の机の高さを検出する機能ではありません。
...
問題が起きたときは、何をしたときに何が起きたかを添えて、田中にログを送ってください。原因の調査や改善に役立ちます。
