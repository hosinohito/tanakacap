...
指の実装版比較：`compare_finger_revision.py --records <保存済み手観測frames.jsonl> --reference-report <compare_hand_mirrorのreport.json> --before-revision <Git版> --output <新規フォルダー>`。比較前のpacket完全再現と左指以外の一致を検査し、before/afterのrender_comparison_videos.py用入力を生成する。ラベルは--before-label/--after-labelで指定できる。入力packetは指定した旧Git版に対応している必要がある。実写を表示せず、推論も再実行しない。
...
- `compare_head_yaw.py --records <全身推論frames.jsonl> --hand-records <手観測frames.jsonl> --reference-report <手比較report.json> --output <新規フォルダー>`：同じ撮影時計を検証し、旧ヨー/固定追従と円周ヨー/可変追従の再生入力を作る。両手の指は現行絶対値化に統一し、頭以外のpacket一致を検査。`render_comparison_videos.py`で描画する。
