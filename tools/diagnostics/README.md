...
- `compare_hand_mirror.py --take <録画フォルダー> --records <同じ録画のframes.jsonl> --output <新規フォルダー>`：元入力と水平反転入力を同じROI/尺度でモデルへ渡し、手の左右とXを復元して比較。元記録の顔/腕を保持、Z/XYのみ移植の診断も保存。`summarize_hand_mirror.py <結果フォルダー>`で集計。正解率ではない。
