...
def run(take, output, limit=None):
    ...
    face_model = SimCCModel('rtmw-l-384', None, 'graph')
    ...
    names = ('separate', 'body3d')
    faces = {name: SharedFace(settings, None, execution_mode='graph') for name in names}
    ...
    try:
        with ExitStack() as stack:
            ...
            for clock, image in images(take, timeline, limit):
                ...
                if roi is not None:
                    xy, scores, _ = face_model.predict(image, roi)
                ...
                for name in names:
                    ...
                    packet = faces[name].update(image, points, confidence, index, now)
        ...
        report['models'] = dict(face=face_model.identity, body=body_model.identity, detector=detector.identity)
...
if __name__ == '__main__':
    ...
    run(args.take, args.output.resolve(), args.limit)
