...
def main():
    ...
    parser.add_argument('--stage',choices=('B','C','D','face','F'),default='B')
    ...
    modes={'F':{'baseline':[], 'split':['--detector-graph']},'B':{'run':['--inference-mode','run'],'binding':['--inference-mode','binding'],
    ...
    for name,extra in modes.items():
        ...
        if args.stage in ('F',): command+=['--inference-mode','graph','--detector-interval','3','--face-source','body3d','--head-pose-mode','pnp_depthmouth']
